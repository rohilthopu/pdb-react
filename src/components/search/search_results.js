...
class SearchResults extends React.Component {
    ...
    getImageLink(result) {
        return "https://api.pad-db.com/static/padimages/portrait/" + String(result.card_id) + ".png";
    }

    getAwokenImageLink(skill) {
        return "http://www.puzzledragonx.com/en/img/awokens/" + String(skill) + ".png"
    }
                    ...
                    <section className="box is-fullheight">
                        ...
                        <table className="table is-fullwidth is-hoverable bottom-space">
                            ...
                            <tr>
                                ...
                                <th className="has-text-centered">AWOKEN SKILLS</th>
                                <th className="has-text-centered">SUPER AWOKEN SKILLS</th>
                            </tr>
                            </thead>
                            <tbody>
                            {
                                    ...
                                    (result) => (
                                        <tr key={result.card_id}>
                                            <td className="has-text-centered">
                                                <img src={this.getImageLink(result)}
                                                     alt="pad_image"
                                                     height="75"
                                                     width="75"
                                                />
                                            ...
                                            <td className="has-text-centered">{this.makeAwokenSkills(result.awakenings_raw)}</td>
                                            <td className="has-text-centered">{this.makeAwokenSkills(result.super_awakenings_raw)}</td>
                                        </tr>
                                    )
                                )
                            }
